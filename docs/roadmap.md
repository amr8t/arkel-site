...
Arkel is in active, early development. The main instance of the network is
...
- **S3-compatible gateway** — use Arkel from existing tools (rclone, restic,
  the AWS SDK) instead of only the `arkel` CLI
- Replication/import tooling to migrate off a cloud provider

## Later: More good stuff

- Public node discovery & reputation (who is fast, reliable, well-connected)
- Node-vetted payout scaling and market-based pricing

This is a rough order, the roadmap will be largely driven by what contributors
actually need.
